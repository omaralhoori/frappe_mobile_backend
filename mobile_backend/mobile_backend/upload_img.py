...
def update_user_image():
	...
	if content:
		result = save_file(fname, content, user)
		if result["file_url"]:
			frappe.db.set_value('User', user, {"user_image": result["file_url"]})
			return result
		else:
...
def get_uploaded_content():
    ...
    if frappe.request.files['filedata']:
        # if "," in frappe.form_dict.filedata:
        #     frappe.form_dict.filedata = frappe.form_dict.filedata.rsplit(",", 1)[1]
        frappe.uploaded_content = frappe.request.files['filedata'].read()#base64.b64decode(frappe.form_dict.filedata)
...
def save_file(fname, content, user):
    ...
    file_data = save_file_on_filesystem(fname, content, user, content_type=content_type, is_private=0)#write_file_method(fname, content, content_type=content_type, is_private=0)
...
def save_file_on_filesystem(fname, content, user, content_type=None, is_private=0):
	fpath = write_file(content, fname, user, is_private)

	if is_private:
		file_url = "/private/files/{0}".format(fname)
	else:
		file_url = "/files/{0}/{1}".format(user,fname)
...
def write_file(content, fname, user, is_private=0):
    ...
    full_path = os.path.join(file_path.encode('utf-8'), fname.encode('utf-8'))
    with open(full_path, 'wb+') as f:
        f.write(content)

    return full_path#get_files_path(fname, is_private=is_private)
